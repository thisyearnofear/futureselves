...
A goal-achievement feature layered on top of the on-device AI foundation (Phases 2–4). The user declares a football dream by speaking it; QVAC STT transcribes, QVAC LLM extracts a structured position/level/description, and the result is saved to Convex. The future self then sends position-grounded voicemails (QVAC LLM → QVAC TTS, all on-device). Three sensor-based drills (reaction time, juggling, sprint) measure progress — the measurement layer is pure sensor/software, **not** AI, keeping it outside the QVAC track's on-device-AI-only rule. QVAC LLM interprets drill trends as a narrative voicemail and suggests alternative positions. All AI runs on-device through the QVAC SDK; no cloud APIs. See Phase 9 below and the Football Path section in `AGENTS.md`.
