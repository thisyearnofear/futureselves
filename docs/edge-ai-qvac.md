...
### Track D — "Football Path" (Tether Developers Cup — QVAC track)
A goal-achievement feature layered on top of the on-device AI foundation (Phases 2–4). The user declares a football dream by speaking it; QVAC STT transcribes, QVAC LLM extracts a structured position/level/description, and the result is saved to Convex. The user then picks a **coach persona** (`tactician` / `enforcer` / `mentor` / `broadcaster`) which conditions the on-device LLM system prompt for every subsequent transmission — today a prompt injection, ready to become a hot-swappable LoRA persona once the QVAC SDK ships `loadAdapter`. The future self then sends position-grounded voicemails (QVAC LLM → QVAC TTS, all on-device). Three sensor-based drills (reaction time, juggling, sprint) measure progress — the measurement layer is pure sensor/software, **not** AI, keeping it outside the QVAC track's on-device-AI-only rule. QVAC LLM interprets drill trends as a narrative voicemail and suggests alternative positions. The viral surface is a FIFA Ultimate Team-style shared card with a `QVAC ON-DEVICE` chip strip, deep-linked through `futureself://challenge?drill=X&target=Y&from=Z`. All AI runs on-device through the QVAC SDK; no cloud APIs. See Phase 9 below and the Football Path section in `AGENTS.md`. The 3-min Cup recording script is `demo/CUP_DEMO.md`.

All four tracks share a single backend integration (one PR each, layering up).
...
### Phase 9 — Football Path (Tether Developers Cup)

A goal-achievement feature built on top of the existing QVAC on-device AI foundation (Phases 2–4). All AI runs on-device through the QVAC SDK — no cloud APIs.

#### Ambition declaration
- ✅ `apps/default/components/football-ambition-declaration.tsx` — speak-your-dream UI (press-to-record → QVAC STT transcription)
- ✅ `apps/default/lib/football-llm.ts` — `extractAmbition` (STT text → QVAC LLM → structured {position, level, description})
- ✅ Structured extraction validated against `positionValidator` (8 football positions + unknown) and saved to Convex (`saveAmbition`)
- ✅ Ambition preview + confirm flow before persisting

#### Football-path transmissions
- ✅ `apps/default/lib/football-llm.ts` — `generateFootballTransmission` (position-grounded voicemail from the future self)
- ✅ `apps/default/components/football-audio-player.tsx` — self-contained QVAC TTS player (file-based playback, audio cache)
- ✅ Transmission text grounded in declared position + level (what being a winger/striker/etc. actually demands)
- ✅ Voice synthesized on-device via QVAC TTS (no ElevenLabs)

#### Sensor-based measurement (NOT AI)
- ✅ `apps/default/lib/drill-utils.ts` — `countJugglePeaks` (accelerometer peak detection), `formatResult`, `isImprovement`, drill metadata
...
- ✅ `apps/default/app/football-drill.tsx` — drill route hosting all three drills + completion flow (start session → complete → recompute trajectory → QVAC LLM interpretation → save narrative)

#### Trajectory interpretation
- ✅ `apps/default/lib/football-llm.ts` — `interpretTrajectory` (drill trend → QVAC LLM → narrative voicemail + alternative position suggestions)
- ✅ `packages/backend/convex/football.ts` — `recomputeTrajectory`, `updateTrajectoryNarrative`, drill session + history APIs
- ✅ Schema tables: `ambitions`, `drillSessions`, `trajectories` (in `schema.ts`)
- ✅ Trajectory narrative rendered on football home with TTS playback

- **Deliverable:** End-to-end football goal-achievement loop — spoken ambition → on-device transmission → sensor drills → on-device trajectory interpretation — fully offline, no cloud AI APIs.
...
- AI tooling context: `AGENTS.md` (QVAC SDK notes, local-fallback ordering, Football Path section for Tether Developers Cup)
