...
const constellationReturnValidator = v.object({
    castMember: castMemberValidator,
    label: v.string(),
    state: v.union(v.literal("lit"), v.literal("dim"), v.literal("locked"), v.literal("quiet")),
    unlockHint: v.string(),
...
const voicePresets: Record<string, { voiceId: string; label: string; description: string }> = {
    ember: {
...
function buildConstellation(persona: PersonaReturn): Array<ConstellationReturn> {
    const hasAvoidance = persona.timelineDivergenceScore >= 4;
    const partnerUnlocked = persona.primaryArc === "love";
    const employeeUnlocked = persona.streak >= 21 && ["purpose", "money"].includes(persona.primaryArc);
    const customerUnlocked = persona.streak >= 30 && persona.primaryArc === "money";
    ...
    return [
        ...
        },
        {
            ...
            label: "Future Child",
            state: persona.futureChildOptIn && persona.streak >= 60 ? "dim" : "locked",
...
function chooseCastMember(context: GenerationContext): CastMember {
    ...
    if (context.persona.streak >= 100 && !hasRecentCast(context, "future_stranger")) return "future_stranger";
    if (context.persona.streak >= 60 && !hasRecentCast(context, "alternate_self")) return "alternate_self";
    if (context.persona.streak >= 30 && litMembers.includes("future_mentor")) return "future_mentor";
    if (context.persona.streak >= 21 && litMembers.includes("future_employee")) return "future_employee";
    if (context.persona.streak >= 7 && litMembers.includes("future_best_friend")) return "future_best_friend";
    if (context.persona.primaryArc === "love" && litMembers.includes("future_partner")) return "future_partner";
    return "future_self";
}

function hasRecentCast(context: GenerationContext, castMember: CastMember): boolean {
    return context.recentTransmissions.some((transmission) => transmission.castMember === castMember);
}
...
function toGeneratedTransmission(value: unknown): GeneratedTransmission | null {
    ...
    if (!("title" in value) || !("text" in value) || !("actionPrompt" in value) || !("cliffhanger" in value)) {
        return null;
    }
...
function fallbackTransmission(context: GenerationContext, castMember: CastMember): GeneratedTransmission {
    ...
    if (castMember === "future_partner") {
        return {
            ...
            actionPrompt: "Make one small repair or loving move before the day closes.",
            cliffhanger: "Tomorrow, I want to tell you the first moment I realized you were actually letting me in.",
        };
    }
    return {
        ...
        cliffhanger: "Tomorrow, I want to tell you what changed the first time you stopped waiting to feel ready.",
    };
}
...
function buildPrompt(context: GenerationContext, castMember: CastMember): string {
    const recentText = context.recentTransmissions
        .map((transmission) => `${transmission.dateKey}: ${transmission.title} — ${transmission.cliffhanger}`)
        .join("\n");
    const choices = context.recentChoices
        .map((choice) => `${choice.dateKey}: ${choice.choice}`)
        .join(", ");
    const threads = context.openThreads.map((thread) => `${thread.title}: ${thread.seed}`).join("\n");
...
function parseAnthropicContent(value: unknown): string | null {
    ...
    const firstTextBlock = content.find((block: unknown) => {
        ...
        return (block as Record<string, unknown>).type === "text" && typeof (block as Record<string, unknown>).text === "string";
    });
    if (typeof firstTextBlock !== "object" || firstTextBlock === null) return null;
...
export const completeOnboarding = authMutation({
    args: {
        ...
        archetype: archetypeValidator,
        firstVoice: castMemberValidator,
        voicePreset: v.union(v.literal("ember"), v.literal("atlas"), v.literal("sol")),
        futureChildOptIn: v.boolean(),
        significantDates: v.array(v.string()),
    },
...
export const saveCheckIn = authMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("checkIns")
            .withIndex("by_userId_and_dateKey", (q) => q.eq("userId", ctx.user._id).eq("dateKey", args.dateKey))
            .unique();
        ...
        const streak = persona.lastCheckInDateKey && isPreviousDateKey(persona.lastCheckInDateKey, args.dateKey)
            ? persona.streak + 1
        ...
        await ctx.db.patch(persona._id, {
            ...
            timelineDivergenceScore: Math.max(0, persona.timelineDivergenceScore - 1),
            updatedAt: now,
        });
...
export const getState = authQuery({
    ...
    handler: async (ctx, args) => {
        ...
        const openThreadDocs = await ctx.db
            ...
            .withIndex("by_userId_and_status", (q) => q.eq("userId", ctx.user._id).eq("status", "open"))
            .take(3);

        return {
            persona,
            todayCheckIn: todayCheckInDoc ? toCheckInReturn(todayCheckInDoc) : null,
            todayTransmission: todayTransmissionDoc ? await toTransmissionReturn(ctx, todayTransmissionDoc) : null,
            recentTransmissions: await Promise.all(
                recentTransmissionDocs.map((transmission) => toTransmissionReturn(ctx, transmission)),
            ),
...
export const getGenerationContext = internalQuery({
    ...
    handler: async (ctx, args) => {
        ...
        const checkInDoc = await ctx.db
            .query("checkIns")
            .withIndex("by_userId_and_dateKey", (q) => q.eq("userId", args.userId).eq("dateKey", args.dateKey))
            .unique();
        const todayTransmissionDoc = await ctx.db
            .query("transmissions")
            .withIndex("by_userId_and_dateKey", (q) => q.eq("userId", args.userId).eq("dateKey", args.dateKey))
        ...
        const openThreadDocs = await ctx.db
            .query("narrativeThreads")
            .withIndex("by_userId_and_status", (q) => q.eq("userId", args.userId).eq("status", "open"))
            .take(3);
        return {
            persona,
...
export const generateDailyTransmission = authAction({
    ...
    handler: async (ctx, args): Promise<{ transmissionId: Id<"transmissions"> | null; generated: boolean }> => {
        const context: GenerationContext | null = await ctx.runQuery(internal.game.getGenerationContext, {
            userId: ctx.userId,
            dateKey: args.dateKey,
        });
        if (!context) throw new Error("Complete onboarding before generating a transmission");
        if (context.existingTransmissionId) {
            return { transmissionId: context.existingTransmissionId, generated: false };
        }
        const anthropicKey = process.env.ANTHROPIC_API_KEY;
        if (!anthropicKey) throw new Error("ANTHROPIC_API_KEY environment variable is required but not set");
        const elevenLabsKey = process.env.ELEVENLABS_API_KEY;
        if (!elevenLabsKey) throw new Error("ELEVENLABS_API_KEY environment variable is required but not set");
        ...
        const response = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 700,
                temperature: 0.8,
                system: "You write emotionally precise narrative transmissions for futureself, a reflective imagination game. Output valid JSON only.",
                messages: [
                    {
                        role: "user",
        ...
        if (!response.ok) {
            ...
            throw new Error(`Anthropic transmission generation failed: ${errorText}`);
        }
        ...
        const generated = toGeneratedTransmission(parsed) ?? fallbackTransmission(context, castMember);

        const ttsResponse = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${context.persona.selectedVoiceId}`, {
            method: "POST",
            headers: {
                accept: "audio/mpeg",
                "content-type": "application/json",
        ...
        const transmissionId: Id<"transmissions"> = await ctx.runMutation(internal.game.storeGeneratedTransmission, {
            userId: ctx.userId,
            dateKey: args.dateKey,
            castMember,
